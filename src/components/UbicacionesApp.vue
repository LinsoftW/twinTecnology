                ...
                <div class="row">
                  <div class="col-md-6 col-xl-6 col-lg-12">
                    <div class="justify-content-between">
                      ...
                      <a @click="EliminarSeleccSucursales()" class="btn btn-danger btn-sm btn-icon-split m-1"
                        :class="disabledMagnitud">
                      ...
                      <a @click="actualizarSucursales()" class="btn btn-success btn-sm btn-icon-split m-1"
                        :class="disabledMagnitud">
                        <span class="icon text-white">
                          <i v-if="actualizaTablaMag == false" class="fas fa-refresh"></i>
                          <i v-else class="fa fa-spinner fa-spin"></i>
                        </span>
                        <span v-if="actualizaTablaMag == false" class="text text-white">{{ Actualizar }}</span>
...
const actualizaTablaMag = ref(false)

const actualizaTablaUbi = ref(false)

const Actualizar = ref('Actualizar')

const ActualizarU = ref('Actualizar')
...
const EliminarSeleccUbicaciones = () => {
  for (let index = 0; index < itemsSelected.value.length; index++) {
    // console.log(itemsSelected.value[index])
    borrarUMedida(itemsSelected.value[index].id, itemsSelected.value[index].attributes.codigo, 2)
  }

}

const actualizarSucursales = async () => {
  actualizaTablaMag.value = true;
...
const actualizarUbicaciones = async () => {
  actualizaTablaUbi.value = true;
  ActualizarU.value = 'Actualizando...'
  let bienActualizado = false;
  disabledU.value = 'disabled'
  // localStorage.removeItem('Carg_datA'); // Articulos
  localStorage.removeItem('Carg_datU'); // Departamentos
  // localStorage.removeItem('Carg_datMe'); // Unidades de medida
  // localStorage.removeItem('Carg_datM'); // Magitudes
  // localStorage.removeItem('Carg_datP'); // Productos
  // localStorage.removeItem('Carg_datS'); // Sucursales
  ...
  // localStorage.removeItem('Carg_datAu'); // Auditoria
  // localStorage.removeItem('Carg_datPe'); // Personas
  localStorage.setItem('Carg_datU', '0'); // Departamentos
  // localStorage.setItem('Carg_datA', '0'); // Articulos
  // localStorage.setItem('Carg_datMe', '0'); // Unidades de medida
  // localStorage.setItem('Carg_datM', '0'); // Magitudes
  // localStorage.setItem('Carg_datP', '0'); // Productos
  ...
  if (localStorage.getItem('userName')) {
    ...
    disabledU.value = '';
    actualizaTablaUbi.value = false;
    ActualizarU.value = 'Actualizar'
    if (bienActualizado == true) {
      successFull("Datos actualizados satisfactoriamente.", "top-end")
    } else {
      ErrorFull("Hubo un error actualizando los datos. Vuelva a intentarlo.", "top-start")
    ...
    Swal.fire({
      ...
      if (result.isConfirmed) {
        ...
        for (let index = 0; index < itemsSelected.value.length; index++) {
          ...
          const response = await EliminarDatos(itemsSelected.value[index].id, 7);
          if (response == null) {
            // Store.cambiaEstado(1);
            correcto = false;
          } else {
        ...
        if (correcto == true) {
          itemsUbicaciones1.value = store.itemsUbicaciones;
          successFull("Ubicaciones eliminadas satisfactoriamente.", "top-end")
          store.cambiaEstado(7);
          disabledU.value = ''
        } else {
          ErrorFull("Error eliminando los elementos seleccionados.", "top-start")
