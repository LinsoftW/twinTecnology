                    ...
                    <select class="form-control form-control-user">
                      ...
                      <option value="">Categorías</option>
                            ...
                            <div class="col-xl-8 col-lg-6 col-md-6">
                                ...
                                <input type="file" id="archivoExcel" class="form-control form-control-user"
                                    @change="onFileChange" accept=".xlsx, .xls" />
                      ...
                      id="loadExcel" @click="loadExcel"><i class="fas fa-upload"></i> Cargar Excel</button>
                            ...
                            <div class="col-xl-2 col-lg-6 col-md-6 col-sm-4">
                                ...
                                <button v-if="cargado"
                                    ...
                                    id="modifyExcel" @click="almacenarDatos()"><i class="fas fa-save"></i> Guardar
                                    datos</button>
                                ...
                                <div class="spreadsheet-container">
                                    ...
                                    <div class="row form-control-user text-center" v-if="editaItem">
                                        ...
                                        <div v-for="(ite, index) in itemEditar.dat" class="input-container m-2">
                                            ...
                                            <input type="text" :value="ite" :key="index" v-if="index == 0" disabled
                                                :id="index" class="form-control form-control-user"
                                    ...
                                    <EasyDataTable :headers="headers2" :items="item" buttons-pagination border-cell
                                        v-model:items-selected="itemsSelected" header-text-direction="center"
                                        body-text-direction="center" @click-row="showRow" :rows-per-page="5">
                                        ...
                                        <template #item-opciones="item">
                                            <button class="btn btn-primary btn-sm btn-circle ml-1" data-toggle="modal"
                                                data-target="#agregaProducto" @click="editar(item)" v-b-tooltip.hover
                                                title="Modificar"><span class="fas fa-edit"></span></button>

                                            <button disabled class="btn btn-danger btn-sm btn-circle ml-1"
                                                @click="borrarU(item)" v-b-tooltip.hover title="Eliminar"><span
                                        ...
                                        <template v-slot:cell="{ column, item, rowIndex }">
                                            <div v-if="editingIndex === rowIndex && editingField === column.value">
                                                ...
                                                <input type="text" v-model="editValue"
                                                    @blur="saveEdit(item, column.value)"
                                                    @keyup.enter="saveEdit(item, column.value)" />

                                            </div>

                                            <div v-else
                                                @dblclick="editCell(rowIndex, column.value, item[column.value])">
...
<template #item-image="item">
    <img src="/inventario.jpg" alt="No image" class="img img-thumbnail" style="width: 70px; height: 70px;" />
</template>
<template #item-opciones="item">
    <div class="operation-wrapper">
        ...
        <div :class="'modal fade ' + showModal" id="filasColumnas" tabindex="-1" role="dialog"
            aria-labelledby="exampleModalLabel" :aria-hidden="activaHide" :arial-modal="activaModal"
            :style="displayModal">
                ...
                <div class="modal-content">
                    ...
                    <div class="modal-body">
                        ...
                        <ol>
                            ...
                            <li><input type="checkbox" name="descripcion" :checked="sidescripcion"
                                    @change="quitardescripcion">
                                Descripción</li>
                            <li><input type="checkbox" name="observaciones" :checked="siobservaciones"
                                    @change="quitarobservacion">
                                Observaciones</li>
...
import {
    successFull
} from './controler/ControlerApp';
import {
    useStoreAxios
} from '@/store/AxiosStore';
import {
    GuardarColecciones,
    obtenerDatos
} from './helper/useAxios';
import {
    ref
} from 'vue';
...
export default {
    ...
    methods: {
        editar(item) {
            this.editaItem = true;
            this.itemEditar.dat = [];
            this.itemEditar.header = []
            // console.log(this.headers2)
            // console.log(this.headers2.length)
            for (let index = 0; index < this.getObjectSize(item) - 1; index++) {
                this.itemEditar.dat.push(item[index])
            }
            for (let index = 0; index < this.headers2.length; index++) {
                // console.log(this.headers2[index].text)
                this.itemEditar.header.push(this.headers2[index].text)
            }
            for (let index = 0; index < this.listados.length; index++) {
                if (this.listados[index][1] == this.itemEditar.dat[1]) {
                    this.posicion = index;
                }
            }
        },
        ...
        guardaCambio() {
            ...
            this.listados.splice(this.posicion, 1, this.itemEditar.dat)
            this.preparar_Guardar(this.listados)
        },
        getObjectSize(obj) {
